...
# ╔═╡ 86693178-7c7f-11f0-3916-db6ac045ae1a
using DifferentialEquations, Random, Statistics, LinearAlgebra, PlutoUI, Plots

# ╔═╡ 7f99c0c0-ca3f-4dc7-a49a-cb9923768643
@bind seed NumberField(1:10_000, default=1234)

# ╔═╡ c97bf841-51ce-40ab-9716-e00d1144f5c1
@bind γ_total_int NumberField(1:1:2000, default=100)

# ╔═╡ 17745080-168a-4687-ab13-10b7f195f5a8
@bind Ω0_int NumberField(1:1:5000, default=500)

# ╔═╡ 1b918fd9-2dcb-496c-8c9e-6ff86725cf99
@bind Δ_int NumberField(-2000:1:2000, default=0)

# ╔═╡ f032c4c1-66c1-4e1f-b412-220886d84ddc
@bind αR_int NumberField(-200:1:200)

# ╔═╡ 3445fc1c-70c4-4ff4-a6e0-3c78d8181bd7
@bind αI_int NumberField(-200:1:200)

# ╔═╡ 6133e8a0-f36c-4f11-b4bf-e40727ed04d7
@bind A_int NumberField(0:1:500, default=10)

# ╔═╡ 496c57e5-06da-48df-bb7e-37b340a2ef1f
@bind σ_int NumberField(0:1:200, default=5)

# ╔═╡ aa740133-14be-44c7-9ef8-a3012b17d0ac
@bind r_pct Slider(0:1:200, default=100, show_value=true)

# ╔═╡ 93d0fcfa-c919-45fb-a460-9127dd9de44b
@bind periods_transient NumberField(0:1:20_000, default=1000)

# ╔═╡ 49088c88-71d5-4419-9689-91315f2e35fb
@bind periods_analyze NumberField(1:1:20_000, default=2000)

# ╔═╡ 39067e6b-e076-4760-a1ac-6e53fa643574
@bind pts_per_period NumberField(50:10:5000, default=200)

# ╔═╡ 58a6c34e-38c4-48d9-865d-fc071818b9ff
@bind do_sweep CheckBox(default=true)

# ╔═╡ e5f783e0-ab7e-45a6-9455-e4c66c3b46c3
@bind sweep_step_pct NumberField(1:1:25, default=5)

# ╔═╡ fa08c697-d76a-43ae-a6a5-0fd74f700aea
begin
    # ----- Convert integers to Float64 (dimensionless) -----
    γ_total = float(γ_total_int)
    Ω0      = float(Ω0_int)
    Δp      = float(Δ_int)
    Ωd      = Ω0 + Δp
    α       = float(αR_int) + 1im*float(αI_int)
    A       = float(A_int)
    σ       = float(σ_int)
    r       = float(r_pct)/100
    γe      = r * γ_total
    ...
    # ----- Time grid -----
    T = 2π/abs(Ωd)                      # period in dimensionless time τ
    tspan = (0.0, (periods_transient + periods_analyze) * T)
    nsave = max(2, Int((periods_transient + periods_analyze) * pts_per_period))
    saveat = range(tspan[1], tspan[2]; length = nsave)

    # ----- Drift & diffusion -----
    signal(τ, Ω, A) = A * cos(Ω*τ)
    ...
    # Additive complex noise: equal power on Re/Im via 2 real channels
    function g!(du, u, p, τ)
        σ = p[7]
		α = p[2]
        du[1,1] = σ           # real channel
        # du[1,2] = 1im * σ * sqrt(abs(imag(α)))     # imaginary channel
		# du[1,2] = 1im*σ
		du[1,2] = sqrt(abs(imag(α)))*σ
    end
    Gproto = zeros(ComplexF64, 1, 2)

    # ----- Stable dt: min(period-based, decay-based cap) -----
    function compute_dt()
        dt_T   = T/pts_per_period
        dt_cap = 1.0 / (20*max(γ_total, 1e-12))  # ~20 steps per 1/γ
        min(dt_T, dt_cap)
    end
    ...
    function solve_once()
        Random.seed!(seed)
        u0 = ComplexF64[0.0]
        p = (Ω0, α, γ_total, γe, A, Ωd, σ)
        prob = SDEProblem(f!, g!, u0, tspan, p; noise_rate_prototype=Gproto)
        dt = compute_dt()
        solve(prob, EM();                 
              dt=dt,
              saveat=saveat,
    ...
    # Ensure t/u lengths match
    t_full = (length(sol.t) == length(sol.u)) ? sol.t : range(tspan[1], tspan[2]; length=length(sol.u))
    a_full = first.(sol.u)

    # ----- Post-transient slice with guards -----
    t_cut = periods_transient * T
    id0 = searchsortedfirst(t_full, t_cut)
    if id0 > length(t_full)
        @warn "Transient window removed all samples; keeping last 20%."
        id0 = max(1, round(Int, 0.8*length(t_full)))
    end
    t_keep = (length(t_full)==0) ? Float64[]    : t_full[id0:end]
    a_keep = (length(a_full)==0) ? ComplexF64[] : a_full[id0:end]
    ...
    E       = abs2.(a_keep)
    s_plus  = A .* cos.(Ωd .* t_keep)
    s_minus = -s_plus .+ sqrt(2γe) .* a_keep
    ϕ       = unwrap_phase(angle.(a_keep))

    if length(t_keep) > 1
        den = max(t_keep[end] - t_keep[1], eps())
        E_mean = mean(E);  E_std  = std(E)
        Pin    = mean(abs2.(s_plus))
        Pout   = mean(abs2.(s_minus))
        Dϕ     = var(ϕ) / den
    else
        @warn "No post-transient samples; stats set to NaN."
        E_mean = NaN; E_std = NaN; Pin = NaN; Pout = NaN; Dϕ = NaN
    end

    # ----- Sweep r in integer percent steps -----
    function sweep_summary(; step_pct=5)
        if !do_sweep; return nothing; end
        rs_pct = collect(0:step_pct:100)
        stats = Vector{NamedTuple}(undef, length(rs_pct))
        for (i, rpc) in enumerate(rs_pct)
            rr   = rpc/100
            γe_r = rr * γ_total
            Random.seed!(seed)
            u0 = ComplexF64[0.0]
            p = (Ω0, α, γ_total, γe_r, A, Ωd, σ)
            prob = SDEProblem(f!, g!, u0, tspan, p; noise_rate_prototype=Gproto)
            dt = compute_dt()
            solr = solve(prob, EM(); dt=dt, saveat=saveat, save_everystep=false, dense=false)

            tr = (length(solr.t) == length(solr.u)) ? solr.t : range(tspan[1], tspan[2]; length=length(solr.u))
            ar = first.(solr.u)

            id0r = searchsortedfirst(tr, t_cut)
            if id0r > length(tr); id0r = max(1, round(Int, 0.8*length(tr))); end
            tr = (length(tr)==0) ? Float64[]    : tr[id0r:end]
            ar = (length(ar)==0) ? ComplexF64[] : ar[id0r:end]

            Er = abs2.(ar)
            splus_r  = A .* cos.(Ωd .* tr)
            sminus_r = -splus_r .+ sqrt(2γe_r) .* ar
            ϕr = unwrap_phase(angle.(ar))

            if length(tr) > 1
                denr = max(tr[end] - tr[1], eps())
                stats[i] = (r=rr, γe=γe_r, γd=γ_total-γe_r,
                            E_mean=mean(Er), E_std=std(Er),
                            Pout=mean(abs2.(sminus_r)), Dϕ=var(ϕr)/denr)
            else
                stats[i] = (r=rr, γe=γe_r, γd=γ_total-γe_r,
                            E_mean=NaN, E_std=NaN, Pout=NaN, Dϕ=NaN)
            end
        end
        stats
    end

    stats = sweep_summary(step_pct=sweep_step_pct)
end


# ╔═╡ b1095492-8cb6-47cd-a234-cfe0ef3030fa
begin 
	  # ----- Plots -----
    default(lw=2)
    plt_time_E   = plot((length(t_keep)>0 ? t_keep .- t_keep[1] : t_keep), E, xlabel="τ since transient", ylabel="|a|²", title="|a|² vs τ")
    # plt_time_out = plot((length(t_keep)>0 ? t_keep .- t_keep[1] : t_keep), abs2.(s_minus), xlabel="τ since transient", ylabel="|s₋|²", title="Output |s₋|² vs τ")
end

# ╔═╡ 5acce9c7-0968-46e2-a302-1a65f9d7908c
plt_time_out = plot((length(t_keep)>0 ? t_keep .- t_keep[1] : t_keep), abs2.(s_minus), xlabel="τ since transient", ylabel="|s₋|²", title="Output |s₋|² vs τ")

# ╔═╡ 469f7598-1480-4170-8335-ea0368442865
begin
	α_det = float(αR_int) + 1im*float(αI_int)

	#Deterministic solve at a given r= gamma_e/gamma
	function solve_current_det(; r=0.5)
		u0 = ComplexF64[0.0]
		γe = r * γ_total
		p = (Ω0, α, γ_total, γe, A, Ωd)
		prob = ODEProblem(f!, u0, tspan, p)
		solve(prob, Tsit5(); saveat=saveat, save_everystep=false, dense=false)
	end

	function analyze_solution_det(sol, r)
		t_full = (length(sol.t) == length(sol.u)) ? sol.t : range(tspan[1], tspan[2]; length=length(sol.u))
		a_full = first.(sol.u) 
		...
		id0 = searchsortedfirst(t_full, t_cut)
		if id0 > length(t_full)
			@warn "Deterministic run: transient removed all samples; keeping last 20%"
			id0 = max(1, round(Int, 0.8*length(t_full)))
		end

		t_keep_det = (length(t_full)==0) ? Float64[] : t_full[id0:end]
		a_keep_det = (length(a_full)==0) ? ComplexF64[] : a_full[id0:end]

		#observables (no noise in S+, only coupling via gamma_e in s- def)
		E = abs2.(a_keep_det)
		s_plus = A .* cos.((Ω0_int+Δ_int) .* t_keep_det)
		s_minus = -s_plus .+ sqrt(2*r*γ_total) .* a_keep_det
		ϕ_det = unwrap_phase(angle.(a_keep_det))

		if length(t_keep_det) > 1
			den = max(t_keep_det[end] - t_keep_det[1], eps())
			return (r=r,
				   E_mean = mean(E),
				   E_std = std(E),
				   Pout = mean(abs2.(s_minus)),
			...
			return (r=r, E_mean=NaN, E_std=NaN, Pout=NaN, Dϕ=NaN)
		end
	end
	# Sweep over r deterministically 
	function sweep_summary_det(; step=0.2)
		rs = collect(0.0:step:1.0)
		stats = Vector{NamedTuple}(undef, length(rs))
		for (i, rr) in enumerate(rs)
			sol = solve_current_det(r=rr)
			stats[i] = analyze_solution_det(sol,rr)
		end
		stats
	end
	stats_det = sweep_summary_det(step=sweep_step_pct/100)
end

# ╔═╡ ac9749c6-493a-4748-9e0c-e6e5f0047fc8
begin
	 md"""
    ### Dimensionless Noisy SDE Sweep (dimensionless, additive noise)

    **Current-r stats (post-transient)**  
    ⟨|a|²⟩ = **$(round(E_mean, sigdigits=6))**,  std = **$(round(E_std, sigdigits=6))**  
    ⟨|s₋|²⟩ = **$(round(Pout, sigdigits=6))**,   ⟨|s₊|²⟩ = **$(round(Pin, sigdigits=6))**  
    D_φ ≈ **$(round(Dϕ, sigdigits=6))** (rad² per dimensionless time)
    """
end

# ╔═╡ 4272e96e-d49c-4134-833b-3bd897fed0b2
begin
	rs_det = [s.r for s in stats_det]
	Emeans_det = [s.E_mean for s in stats_det]
	Pouts_det = [s.Pout for s in stats_det]
	Dphis_det = [s.Dϕ for s in stats_det]
end

# ╔═╡ 15846d65-319b-4899-ad54-fd388d83b215
begin 
	 plt_sweep1 = plt_sweep2 = plt_sweep3 = nothing
    if stats !== nothing
        rs      = [s.r      for s in stats]
        Emeans  = [s.E_mean for s in stats]
        Pouts   = [s.Pout   for s in stats]
        Dphis   = [s.Dϕ     for s in stats]
        plt_sweep1= plot(rs, Emeans, xlabel="r = γₑ/γ", ylabel="⟨|a|²⟩", title="⟨|a|²⟩ vs coupling", marker=:circle)
        plt_sweep2 = plot(rs, Pouts,  xlabel="r = γₑ/γ", ylabel="⟨|s₋|²⟩", title="Avg output power vs coupling", marker=:circle)
        plt_sweep3 = plot(rs, Dphis,  xlabel="r = γₑ/γ", ylabel="D_φ (rad²/τ)", yscale=:log10, title="Phase diffusion vs coupling", marker=:circle)
    end
    plot(plt_sweep1, plt_sweep2, plt_sweep3)
end

# ╔═╡ b44248a4-5efd-46ad-89df-35c4c2342bac
md"""
    ### Deterministic sweep (no stochastic noise)

    Plots show how the steady-state metrics vary with **r = γₑ/γ** in the **noise-free** model.
    """

# ╔═╡ b805b433-4544-43ec-89c7-714d77dcb9c3
plt_det1 = plot(rs_det, Emeans_det, xlabel="r = γₑ/γ", ylabel="⟨|a|²⟩", title="Deterministic ⟨|a|²⟩ vs coupling", marker=:circle)

# ╔═╡ 790c02d9-b2e7-4ad9-8e93-e05f3208dd53
plt_det2 = plot(rs_det, Pouts_det,  xlabel="r = γₑ/γ", ylabel="⟨|s₋|²⟩", title="Deterministic Avg. output power", marker=:circle)

# ╔═╡ 76b0302e-36bb-437f-bc64-183ede434989
plt_det3 = plot(rs_det, Dphis_det,  xlabel="r = γₑ/γ", ylabel="D_φ (rad²/τ)", yscale=:log10, title="Deterministic phase diffusion", marker=:circle)
...
manifest_format = "2.0"
project_hash = "37cf1260d0ea2c17d1f1767e3fac2a52dda793b9"
