...
# ╔═╡ 172c439a-6d75-11f0-187f-57b367c18e8b
using Plots, PlutoUI, BifurcationKit, LinearAlgebra, DifferentialEquations, PolynomialRoots

# ╔═╡ 7c2c70b5-1f1d-49e8-89c5-d6954eabc185
md"""
** You can modify the parameters values until you the bifurcation appears. These values are set to integers but potentially can use the ratios between the variables **
"""

# ╔═╡ c37bb6cb-a011-4c98-985d-8aca16da52fc
md" Kerr coefficient α: $(
@bind α Slider(0.1e1:1e1,show_value=true))
"

# ╔═╡ 1e394094-3a58-4c9e-b0d7-a12dc7610b16
md"ω (rad/s): $(@bind ω Slider(2π:2π*1e1, show_value=true))"

# ╔═╡ 67700f99-d616-4c94-a66b-a918dd6c5e1e
md" γ: $(@bind γ Slider(0.1e1:1e1, show_value=true))"

# ╔═╡ 9841a4f7-674c-40de-8f9e-01f460b7ee1d
md" Amplitude: $(@bind S Slider(0.1e1:0.1e1:1e1, show_value=true))"

# ╔═╡ 3aafa0dc-0dca-4e33-8957-950bff736eb5
begin
	function Fcubic(x,p)
		(;α,ω,γ,γe,S, D) = p
		c1 = α^2
		...
		x.^3*c1 .+ x.^2*c2 .+ x*c3 .+ c4 .-c5
	end
	
	par_ls = (α=0., ω, γ, γe=γ/2.0, S, D=3.0)
	u0 = zeros(1)
	probl = BifurcationProblem(Fcubic, u0, par_ls, 1; record_from_solution= (x,p; k...)->x[1])
	...
	plt = plot(br1; xlabel="α", ylabel="x", legend=false, title="TCMT Bistability Bifurcation")

	md"""
	Equation: α²x³ - 2αωx² + (4γ²+ω²)x - 2γ|S|² - W*D
	"""
end

# ╔═╡ 0864c864-e3aa-4546-be51-59ab923ffb14
plt

# ╔═╡ e517eb92-e926-46dd-95ae-f2a5b8b32e38
display(br1.specialpoint)

# ╔═╡ 21f4d1d3-4bfa-4338-82d3-9f7e475f00da
# ╠═╡ disabled = true
#=╠═╡
begin
		c1 = α^2
...
manifest_format = "2.0"
project_hash = "4d7e6001a7b124816fda0438b43dd45832aeaf5d"
